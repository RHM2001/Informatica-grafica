...
string FiltroVideo= "Todos los formatos (*.avi *.mpg *.mp4 *.wmv *.mov);;Archivos AVI (*.avi);;Archivos MPG (*.mpg *.mp4 *.mpeg);;Archivos WMV (*.wmv);;Archivos MOV (*.mov);;Otros (*.*)";
...
void capturar_de_camara ()
{
    // Con cap(0) se coge de camara si se le indica la ruta lo coge de archivo
    VideoCapture cap(0);
    if (cap.isOpened()) {
        Mat img;
        int tecla;
