...
void cb_punto (int factual, int x, int y)
{
    ...
    if (difum_pincel==0)
        ...
        // Comprobaciones por si se sale para que no pete
        if (roi.x < 0) {
            roi.width+= roi.x;
            posx += roi.x;
            roi.x=0;
...
Scalar color_arco_iris()
{
    ...
    switch (estado) {
        ...
        case 2:
            ...
            if (color_actual.val[0] >= 255)
                estado++;
        break;

        case 3:
          color_actual.val[1]-=salto;
          if (color_actual.val[1] <= 0)
              estado++;
        break;

        case 4:
            color_actual.val[2]+= salto;
            if (color_actual.val[2] >= 255)
                estado++;
        break;
        case 5:
        color_actual.val[0] -= salto;
        if (color_actual.val[0] <= 0)
            estado=0;
        break;
    }
...
//Recibe el evento, la posicion x e y, flags y el puntero al array de ventanas
void callback (int event, int x, int y, int flags, void *_nfoto)
{
...
void ver_rotar_escalar_imagen(int nfoto, int nres, double angulo, double escala, bool guardar)
{
    assert(nfoto >= 0 && nfoto < MAX_VENTANAS && foto[nfoto].usada);
    Mat entrada = foto[nfoto].img;
    Mat imgRotadaEscalar;
    rotar_angulo(entrada, imgRotadaEscalar, angulo, escala, 1);
    if (guardar) {
        crear_nueva(nres, imgRotadaEscalar);
    }
    else {
        imshow("RotarEscalar", imgRotadaEscalar);
    }
}
...
Mat op_mat_sat_lum (Mat img, double sat, double lum, int mat)
{
    ...
    split(hls, canales);
    Mat im16;
    canales[0].convertTo(im16, CV_16S, 1, mat);
...
void ver_mat_sat_lum (int nfoto, double sat, double lum, int mat, bool guardar)
{
    Mat res = op_mat_sat_lum(foto[nfoto].img, sat, lum, mat);

    if (guardar) {
        res.copyTo(foto[nfoto].img);
        foto[nfoto].modificada = true;
    }
    else imshow(foto[nfoto].nombre, res);
...
void ver_bajorrelieve(int nfoto, int nres, double ang, double grado, int nfondo, bool guardar)
{
    ...
    cvtColor(foto[nfoto].img, gris, COLOR_BGR2GRAY);
    Mat rotada;
    rotar_angulo(gris, rotada, ang, 1.0, 1);
    Mat sobel;
    Sobel(rotada, sobel, CV_8U, 1, 0, 3, grado, 128, BORDER_REFLECT);
    rotar_angulo(sobel, rotada, -ang, 1.0, 0);
...
{
    Mat copia = foto[nfoto].img(foto[nfoto].roi).clone();

    cvtColor(copia, copia, COLOR_BGR2RGB);
    QClipboard *clipboard = QGuiApplication::clipboard();
    QImage qImage(copia.data, copia.cols, copia.rows, copia.step, QImage::Format_RGB888);
    qImage = qImage.convertToFormat(QImage::Format_ARGB32);
    clipboard->setImage(qImage, QClipboard::Clipboard);
...
void pegar_del_portapapeles(int nres)
{
    QClipboard *clipboard = QGuiApplication::clipboard();
    QImage imq = clipboard->image();

    if (imq.isNull())
        QMessageBox::information(w, "Información", "No hay ninguna imagen copiada en el portapapeles");
    else {
...
void convertir_a_color_falso(int nfoto, int nueva, int modo, bool guardar)
{
    ColormapTypes paletas[4] = {COLORMAP_RAINBOW, COLORMAP_COOL, COLORMAP_PLASMA, COLORMAP_TWILIGHT_SHIFTED};
    ...
    Mat imgC;
    applyColorMap(gris, imgC, paletas[modo]);

    if (guardar)
        crear_nueva(nueva, imgC);
    else
        imshow("Imagen Color Falso", imgC);
}

//---------------------------------------------------------------------------

void modelo_de_color(int nfoto, int nueva, int modelo, bool guardar)
{
    int modelos[5] = {COLOR_BGR2RGB, COLOR_BGR2HLS, COLOR_BGR2HSV, COLOR_BGR2XYZ, COLOR_BGR2YUV};

    Mat imgM;
    cvtColor(foto[nfoto].img, imgM, modelos[modelo]);

    if (guardar)
        crear_nueva(nueva, imgM);
    else
        imshow("Nuevo modelo de color", imgM);
}
...
void ver_perspectiva(int nfoto1, int nfoto2, Point2f pt1[], Point2f pt2[], bool guardar)
{
    ...
    if (guardar) {
        ...
        for (int i = 0; i < 4; i++)
            circle(res, pt2[i], 10, CV_RGB(0, 255, 0), -1);
...
void morfologia_matematica(int nfoto, int nres, int nit, int modo, bool guardar)
{
    ...
    if (modo == 0)
        dilate(img, morfo_mat, kernel, Point(-1, -1), nit);
    else if (modo == 1)
        erode(img, morfo_mat, kernel, Point(-1, -1), nit);
    else if (modo == 2)
        morphologyEx(img, morfo_mat, MORPH_OPEN, kernel, Point(-1, -1), nit);
    else
        morphologyEx(img, morfo_mat, MORPH_CLOSE, kernel, Point(-1, -1), nit);

    if (guardar)
        crear_nueva(nres, morfo_mat);
    else
        imshow("Morfología matemática", morfo_mat);
...
void balance_blancos(int nfoto, int nres)
{
    ...
    Scalar valor_medio = mean(gris);

    double ganancia = 255.0 / valor_medio[0];

    Mat imagen_balanceada;
    foto[nfoto].img.convertTo(imagen_balanceada, -1, ganancia);

    imshow("Imagen balanceada de blanco", imagen_balanceada);
}
